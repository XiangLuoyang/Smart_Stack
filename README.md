...
1. `POST /api/orders/preview` — 生成 60 秒有效的订单预览(不产生订单)
2. `POST /api/orders/confirm` — 验证预览后原子创建订单+预留+撮合

### A 股规则

- 主板涨跌停 ±10%,创业板/科创板 ±20%,ST ±5%
- 买入必须为 100 股整数倍;卖出零股仅限清仓
- 停牌/过期行情(>30s)/价格越界拒绝
- T+1 可卖:当日买入次日方可卖出(FIFO 消耗结算批次)
...
- **MODEL**: 方向准确率、MAE、RMSE、区间覆盖率
- **SCREEN**: Top10 等权收益、超额收益
- **USER**: 用户判断准确率(案例方向 vs 实际)
- **EXECUTION**: 执行增量(实际执行 vs 决策预期)

### 模型晋升门控

新模型须满足:方向准确率 ≥ 45%、MAE ≤ 5%、样本 ≥ 30、超额收益为正。
未通过门控保持挑战者状态,不替换主力模型。

## Phase 5: 本地产品化

### 启动预检

应用启动前验证:Python ≥ 3.12、数据目录可写、LLM 配置(缺失为 DEGRADED 不阻塞)。

### 健康检查

`GET /api/health` 返回组件级状态(OK/DEGRADED/FAILED)。
...
# 恢复(自动创建 pre-restore 备份)
.\scripts\restore.ps1 -BackupFile "smartstack_20260722_160000.db"

# 健康检查
.\scripts\health.ps1
```

备份包含 SQLite integrity_check + SHA-256 校验 + JSON manifest。

### 结构化日志

JSON 格式日志,自动脱敏含 KEY/TOKEN/SECRET/PASSWORD 的字段。
后台任务记录运行 ID、状态、耗时、错误摘要和重试次数。