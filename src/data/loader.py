...
class StockDataLoader:
    ...
    def load_stock_data(
        ...
        try:
            ...
            self.last_source_errors = self.smart_loader.get_source_errors()

            if df.empty:
                logger.error(f"无法获取股票 {stock_code} 的数据: {self.last_source_errors}")
            ...
            self.last_source_errors = {'exception': str(e)}
            return pd.DataFrame(), ''
